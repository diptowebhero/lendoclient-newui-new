...
function CardNft({ data, size,userAdd }) {
  ...
  // console.log({fileUrl})
  // console.log({data})
  console.log({username, avatar, publicAddress})
  return (
          ...
          <div className="content">
            ...
            <div className="information">
              <Row justify="space-between">
                <Col>{renderType(type, unitPrice, assetPrice)}</Col>
                <Col>{renderTime(expiration, type)}</Col>
              </Row>
            </div>
          </div>
        </div>
...
export default function NftList(props) {
    ...
    return data.map(item => {
      return (
        <div className="nft-item" key={item.id}>
          <CardNft data={item} userAdd={userAdd} size={size}></CardNft>
        </div>
      );
    });
  }
  return (
    <Style size={size}>
      {!isEmpty(data) ? (
        <div className="nft-list">{renderFarm()}</div>
      ) : (
        <Empty />
      )}
    </Style>

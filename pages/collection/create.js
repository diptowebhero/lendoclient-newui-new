...
import { categories, networks } from "@src/data";
...
export default function CollectionCreate() {
  ...
  const onSubmit = async values => {
    try {
      ...
      values.paymentToken = network.currency;
      const response = await postRequest(API_URL_CRUD_COLLECTION, values);
      globalLoading.set(false);
      message("success", t("collection has been created successfully!"));
      router.replace({
  ...
  return (
    <Style>
      <Seo title="Create Collection" desc="" isHome={false} />
      <Mainlayout>
        <div className="container pt40 pb40">
          {network.blockChain ? <Form
            onFinish={onSubmit}
            form={form}
            layout="vertical"
            initialValues={{
              blockChain: network.blockChain,
              paymentToken: network.currency,
              isSensitive: false,
            }}
          >
            <div className="main-form-title pb40">
              <h1 className="title">{t("Create a Collection")}</h1>
              <hr/>
            </div>
            <div className="">
              <TextFrame
            ...
            >
              ...
              {network.blockChain ? <>
                <Select size="large" value={network.chainId} onChange={(e) => switchNetwork(e)}>
                  {networks.map((item, i) => (

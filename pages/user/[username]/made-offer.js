...
export default function AccountOther(props) {
  ...
  const offerItems = get(collectedData, "data.madeOffer", []);
  ...
  const columns = [
    ...
    },
    {
      ...
      dataIndex: "to",
      key: "to",
      render: (to = "", { toPublicAddress }) => (
        <Link href={ROUTE_ACCOUNT_OTHERS.replace(":username", toPublicAddress)}>
  ...
  function onChangePagination(page) {
    router.replace({
      pathname: ROUTE_ACCOUNT_OTHERS_MADE_OFFER.replace(
        ":username",
        usernameParam
      ),
            ...
            <div className="profile-menu">
              <ul>
                ...
                <li>
                  ...
                  >
                    <a className="active">{t("offers made")}</a>
                  </Link>
                </li>
                <li>
                  <Link
  ...
  try {
    [profileApiResponse, profileItemsApiResponse] = await Promise.all([
      ...
      (profileItemsApiResponse = await getRequest(
        `${API_URL_PROFILE}/${username}/made-offer`,
        query
      )),
    ]);
  } catch (e) {

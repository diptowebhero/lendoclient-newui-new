...
export default function AccountOtherFavorites(props) {
  const { profileData, favoritesData, usernameParam } = props;
  console.log(favoritesData, "sdfsdf");
  ...
  const total = get(favoritesData, "metadata.total", 0);
  const offset = get(favoritesData, "metadata.offset", 1);
  const favoritesItems = get(favoritesData, "data.favourites", []);

  function onEnter(values) {
    router.replace({
      pathname: ROUTE_ACCOUNT_OTHERS_FAVORITES.replace(
        ":username",
        usernameParam
      ),
            ...
            <div className="pb40">
              ...
              >
                ...
                <div className="list pt40 pb40">
                  <ProfileNftList data={favoritesItems} />
                </div>
                <Pagination
...
export const getServerSideProps = async ({
  ...
  try {
    [profileApiResponse, profileFavoritesApiResponse] = await Promise.all([
      ...
      (profileFavoritesApiResponse = await getRequest(
        `${API_URL_PROFILE}/${username}/favourited`,
        query
      )),
    ]);
    console.log(profileFavoritesApiResponse);
  } catch (e) {
  ...
  return {
    props: {
      ...
      favoritesData: get(profileFavoritesApiResponse, "data", {}),
